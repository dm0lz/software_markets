class ScrapCapterraMarketCompaniesJob < ApplicationJob
  ...
  def process_page(market_provider, url)
    return unless market = Scraper::PageEvaluatorService.new(url).call(parse_market)
  ...
  def create_domain(company, redirect_url)
    ...
    fetched_domain = if redirect_url == "#"
      ...
      Scraper::PageEvaluatorService.new("#{CAPTERRA_BASE_URL}#{redirect_url}", playwright_options).call(get_domain) || example_domain
    end
